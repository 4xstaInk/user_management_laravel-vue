<template>
    ...
    <div class="card mt-3">
    <div class="card-body">
    <h1 class="text-center"><i class="fas fa-edit"></i> Edit User </h1>
    <h3><button class="btn btn-outline-primary btn-sm" @click="$emit('view-dashboard')"><i class="fas fa-chevron-left"></i> Back</button> 
    <hr>
    <small class="text-muted"><i class="fas fa-user"></i> {{user.name}}</small></h3>
    <hr>
...
export default {
    ...
    methods:{
        ...
        axios.post('/data/users/'+ this.user.id,{
            ...
            this.$emit('user-updated', response.data.user.name + "'s profile has been updated BIYATCH!!!!!")
        }).catch(errors=>{
            console.dir(errors)
